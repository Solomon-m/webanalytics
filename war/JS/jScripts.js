 ...
 */
  ...
  $(function()
	...
	// call schedule email
	  ...
	  if(!localStorage.getItem("AccountDetails")){
		  console.debug("1. Calling /geAccountInfo from Js");
		  $.ajax({
           ...
           .done(function(msg){       
        	   console.debug("2.Sucess... calling loadAccountDetails");
	   ...
	   */
         function loadAccountsDetails(data){
        	 console.debug("3.Inside loadAccountDetails method");
          ...
          * Get the Properties detail of coressponding Account
 			Start       */         
                $('#accountName').change(function(){
                	console.debug("4. Inside accountname dropdown...");
                	...
                	if(!localStorage.getItem(acc_data)){    
                		console.debug("5. Calling ajax method for /getPropertyInfo");
                ...
                function ajaxmethod(urlvalue,acc_detail,propName,acc_data){
                	console.debug(" 6.Inside ajaxmethod for(Properties or profile)");
                	$.ajax({
                		type:"POST",
                		url:urlvalue,    
                		dataType: "json",
                ...
                function displayproperties(data,propName){
                	console.debug("7. Displayproperties method");
                	$property = propName;
     ...
     */            
                ...
                $('#propName').change(function(){
                	console.debug("8. Inside propName change method");
                	$('body').css('cursor','wait');
                	...
                	if(!localStorage.getItem(properties_data)){
                		console.debug("9.Calling /getProfileInfo method");
        ...
        */         
                ...
                $('#profName').change(function(){
                	console.debug("10. Profname change method..");
	    ...
	    */
	      	  ...
	      	  $('#saveprofile').click(function(){
	      		  console.debug("Calling saveProfile() method...");
	      		  saveprofile();
	      	  });       
         ...
         */
				...
				{  
					console.debug("Inside (Getdata) method....");
					$('body').css('cursor','wait');	
					if(getQueryString()){					    
						if(queryString.dimensions.substring(0,3) =="ga:" || queryString.metrics.substring(0,3) =="ga:"  || queryString.filter.substring(0,3) =="ga:"  || queryString.sort.substring(0,3) =="ga:" )
							...
							if(queryString.dimensions.split(',').length < 8 || queryString.dimensions.split(',').length < 8){
								    ...
								    console.debug("Calling /getdataAjax url...");
										$.ajax(
							    		  {
										...
										.done(function (gdata) 
							    		  { 
											console.debug("successfully got GA data");
											console.debug("GA data is "+gdata);
											JSONDATA = gdata;											
											if(profileindex){
							      		  ...
							      		  .fail(function (msg){
							      			  console.debug("Something went wrong while getting GA data..");
							      			$('#totalcountrep').text('');
  ...
  function getQueryString(){
	  console.debug("getQueryString.. method()");
  ...
  function dimensionadd(dimension,select_box,optionvalue){
	  console.debug("Add dimension method....()");
   ...
   function appendtoprofilelist(data){	 
	   console.debug("Appendtoprofilelist... method()");
   ...
   function generateTable(data,range){
	...
	function saveprofile(){ 		
		  console.debug("Inside saveprofile method()...");
  		if(getQueryString()){
  		...
  		if(profileName != "" && profileName != null){  			
  				...
  				console.debug("Calling profileAdd method()...");
   ...
   //function start for csv download
	...
	/**
	 * Original profileadd Method created by shashank
	 */
	function profileadd(queryString){
		console.debug("Inside profileAdd method()...");
		var queryIndex = Math.floor((1 + Math.random()) * 0x10000).toString(16);
		console.debug("Checking what comes in to profileadd::"+JSON.stringify(queryString));
//	    var queryString1 = [];
//      
//		for(var i in queryString){   // for each starts here
//			  queryString1.push([i,queryString[i]]);
//		    var tqueryId;
//		    var tmetrics;
//		    var tdimensions;
//		    var tstartDate;
...
//		    var tprofileName;
//
//		 console.debug("Checking queryString:::"+queryString1);
//		        for(var i= 0; i<queryString1.length; i++){
//		            console.debug("*********************************");
//		            console.log(":::checking ::"+JSON.stringify(queryString1[i][0]));
//		            console.log(":::checking ::"+JSON.stringify(queryString1[i][1]));
//		         
//		                tqueryId = queryString1[i][1].queryId;
//		                tmetrics = queryString1[i][1].metrics;
//		                tdimensions = queryString1[i][1].dimensions;
//		                tstartDate = queryString1[i][1].startDate;
//		                tendDate = queryString1[i][1].endDate;
//		                tsegement = queryString1[i][1].segment;
//		                tfilter = queryString1[i][1].filter;
//		                tsort = queryString1[i][1].sort;
//		                tmaxResults = queryString1[i][1].maxResults;
//		                tprofileName = queryString1[i][1].profileName;
//		                
//		               
//		                
//		               
...
//		        console.debug("Checking for QueryId:::::::::"+tqueryId);
//                console.debug("Checking for metrics:::::::::"+tmetrics);
//                console.debug("Checking for dimensions::::::"+tdimensions);
//                console.debug("Checking for startdate:::::::"+tstartDate);
//                console.debug("Checking for enddate:::::::::"+tendDate);
//                console.debug("Checking for segement::::::::"+tsegement);
//                console.debug("Checking for filter::::::::::"+tfilter);
//                console.debug("Checking for sort::::::::::::"+tsort);
//                console.debug("Checking for maxResults::::::"+tmaxResults);
//                console.debug("Checking for profileName:::::"+tprofileName);
//                console.log("***Outside for loop***");
//                var tqueryString= {};
//                tqueryString.queryId = tqueryId;
//                tqueryString.metrics = tmetrics;
//                tqueryString.dimensions = tdimensions;
//                tqueryString.startDate = tstartDate;
//                tqueryString.endDate = tendDate;
//                tqueryString.segment = tsegement;
//                tqueryString.filter = tsegement;
//                tqueryString.sort = tfilter;
//                tqueryString.maxResults = tmaxResults;
//                tqueryString.profileName = tprofileName;
//                var tqueryIndex = Math.floor((1 + Math.random()) * 0x10000).toString(16);
//                tqueryString.profileIndex = tqueryIndex;
//                console.debug("Checking new TqueryString change"+JSON.stringify(tqueryString));
                
                
		
		
			queryString.profileIndex = queryIndex;
			console.debug("Checking after adding queryIndex to queryString::"+JSON.stringify(queryString));	
			//console.log("Checking  queryIndex::"+queryIndex);
			console.debug("Calling /storeprofiledata url....");
		...
		//	console.log(queryString);	
			//console.debug("Checking Query:::"+queryString);
			//var tempstringtocheck = JSON.stringify(queryString);
			//console.debug("checking query after Stringfy::"+tempstringtocheck);
			console.debug("Checking tmpobj before appendtofilelist:::"+JSON.stringify(tmpobj));
	...
	/**
	 * Method edited by solomon with shashank's original code
	 * @param queryString
	 */
	//Solomon's method starts
//	function profileadd(queryString){
//		console.debug("Inside profileAdd method()...");
//		var queryIndex = Math.floor((1 + Math.random()) * 0x10000).toString(16);
//		console.debug("Checking what comes in to profileadd::"+JSON.stringify(queryString));
...
//		
//			//queryString.profileIndex = queryIndex;
//			console.debug("Checking after adding queryIndex to queryString::"+JSON.stringify(tqueryString));	
//			//console.log("Checking  queryIndex::"+queryIndex);
//			console.debug("Calling /storeprofiledata url....");
//
//			$.ajax({
...
//				url:'/storeprofiledata',
//				contentType:"application/json",
//				data:JSON.stringify(tqueryString)
//			}).done(function (reponse){
//				$('#saveprofile').notify( "Success","info",  { position:"bottom" } 				);
//			//	console.log(response);
//				
...
//			});
//			
//			if(tqueryString.profileName != undefined){
//			var qobj = new Object();
//			var tmpobj = new Object();
//			var size = 0;
//													
...
//				qobj = JSON.parse(localStorage.getItem('userlist'));
//									
//				qobj[tqueryString.profileName]=tqueryString;
//			}
//			tmpobj[queryIndex]=tqueryString;
//			profileindex = tqueryIndex;
//			tqueryString.queryIdFlag = tqueryIndex;
//		//	console.log(queryString);	
//			//console.debug("Checking Query:::"+queryString);
//			//var tempstringtocheck = JSON.stringify(queryString);
//			//console.debug("checking query after Stringfy::"+tempstringtocheck);
...
function csvDownload(data,ReportTitle){
  //  console.log(data);
	console.debug("Inside csvDownload method()....");
	//console.debug("Checking CSV data"+data);
 ...
 function downloadlink(data,dataformat){
	 console.debug("Inside downloadlink method()...");
 ...
 /**
  * Getting All Queries for localstore and exporting as a JSON file
  * by solomon
  */
//   $(document).ready(function(){
////	    var getalluserprofileradio="";
////	    var selected = $("#GetAllUserdiv input[type='radio']:checked");
////	    if (selected.length > 0) {
////	    	getalluserprofileradio = selected.val();
////	    }
////	   console.log("Checking selected value:"+getalluserprofileradio);
//	   $('#getalluserprofile').click(function() {
//		   if($('#getalluserprofile').is(':checked')) { alert("it's checked"); }
//		});
//	   
////  	 $('getalluserprofile').click(function(){
////  		 alert("Inside click..getalluserprofile");
////  		 console.log("Inside click .. getalluserprofile");
////  	 },function(){
////  		 alert("Inside callback function...getalluserprofile");
////  		 console.log("Inside callback function.. getalluserprofile");
////  	 });
//  	 
//   });
// (function(){
...
//				temp=temp.substring(0,temp.length-1);	
//				localStorage.setItem("userlist",temp.concat(','+data.substring(1)));	
				tempstring = JSON.parse(temp);
				tempstring1 = JSON.stringify(temp);
//				console.debug("JSON data is::::::"+tempstring);
//				console.log(tempstring);
//				console.log("======stringify data=====");
//				console.log(tempstring1);
//				console.log("=======escaped data=====");
//				escapeddata = 'data:text/json;charset=utf-8,' + escape(tempstring1);
				//var espdata = escape(temp);
				//console.log(escapeddata);
				//console.log(espdata);
				
				 downloadlink(temp,'json');
